...
- **`writ-lower`** — AST → AST pipeline transforms shared by the checkers and every back end. **Linking** flattens a multi-module program into one module (qualifying names, rewriting cross-module calls) so the checkers analyze the same artifact the back ends run — a cross-module call is an ordinary call, checked like any other — and injects the **prelude** (the standard sum types `Option` / `Result`, unless the program declares its own of that name). **Lowering** desugars contract clauses (`requires` / `ensures`) into the shared `Stmt::Check` form **once**, so contract semantics live in exactly one place. Depends on the AST only.
