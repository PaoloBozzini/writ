...
fn program(src: &str) -> writ_cli::Program {
    ...
    writ_cli::Program { modules, root: "main".to_string() }
}
...
fn the_check_step_does_not_execute_the_program() {
    ...
    assert!(writ_cli::check(&p).is_empty(), "check must not execute the program");
...
fn there_is_no_ambient_filesystem_authority() {
    ...
    let codes: Vec<String> =
        writ_check::check_types(&p.modules["main"]).iter().map(|d| d.code.clone()).collect();
    assert!(codes.contains(&"T0003".to_string()), "unknown function, got {codes:?}");
    assert!(writ_cli::run(&p).is_err(), "running it fails closed, touching nothing");
}

#[test]
fn there_is_no_ambient_network_authority() {
    let p = program("fn main() { http_get(\"http://example.com\"); }");
    assert!(writ_cli::run(&p).is_err(), "no network primitive exists to reach");
}
